...
def getCHist():
    chstFilePath=getPathToInputVideos("out_file.chst")
    return parseInput({'fileName':chstFilePath,'filePos':0,'framePos':1,'cellPos':2,'dataPos':3})

def getSift():
    siftFilePath=getPathToInputVideos("out_file.sift")
    return parseInput({'fileName':siftFilePath,'filePos':0,'framePos':1,'cellPos':2,'dataPos':3})

def getMVect():
    import processMvect
    mvectPath=dirname(getPathToInputVideos("out_file.mvect"))
    mvectFilteredPath=processMvect.filterMvect(mvectPath)
    return parseInput({'fileName':mvectFilteredPath,'filePos':0,'framePos':1,'cellPos':2,'dataPos':3})
